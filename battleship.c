...
int main()
{
   int ships, eneShips, stage, actRow, actCol, key, eneRow = -1, eneCol = -1, rep, game, seg;
   ...
   do
   {
      ...
      do
      {
         ...
         do
         {
            fflush(stdin);
            key = getch();
         } while (key != ESC && key != UP && key != DOWN && key != RIGHT && key != LEFT && key != ENTER);
         // Movement
         if (key == UP)
            if (actRow > 0)
               actRow--;
         ...
         if (stage == 1)
         {
            ...
            // showOceanXY(oceanShip, ROWS, COLUMNS, 60, 10, actRow, actCol); //Uncomment for see behind the scene
         ...
         else if (stage == 2)
         {
            if (key == ENTER)
            {
               ...
               {
                  // showOceanXY(tempOcean, ROWS, COLUMNS, 90, 10, actRow, actCol);//Uncomment for see behind the scene
                  ...
                  if (eneShips)
                  {
                     ...
                     {
                        // showOceanXY(eneOcean, ROWS, COLUMNS, 120, 10, eneRow, eneCol);//Uncomment for see behind the scene
...
/*
   Función:    randRange
   Argumentos: int a, primer valor (rango inferior).
               int b, segundo valor (rango superior).
   Objetivo:   Crear un número aleatorio.
